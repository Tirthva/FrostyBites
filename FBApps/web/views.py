...
def customer_support_view(request):
    ...
    if request.method == 'POST':
        ...
        if response.status_code == 201:
            ...
            messages.success(request, f"Successfully submitted your request with ID: {created_request['request_id']}")
            return redirect('customer_support_view')
        else:
            messages.error(request, "Failed to submit your request. Please try again.")
    ...
    if response.status_code == 200:
        ...
        messages.error(request, "Failed to retrieve customer requests.")
        return render(request, 'web/customer_support.html', {'customer_requests': [], 'total_requests': 0})
